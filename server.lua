...
local Webhook = '' ---add discordwebhook
...
QBCore.Functions.CreateCallback("sadiqBilling:GetInvoices", function(source, cb)
	local xPlayer = QBCore.Functions.GetPlayer(source)

	MySQL.Async.fetchAll('SELECT * FROM sadiqBilling WHERE receiver_identifier = @identifier ORDER BY CASE WHEN status = "unpaid" THEN 1 WHEN status = "autopaid" THEN 2 WHEN status = "paid" THEN 3 WHEN status = "cancelled" THEN 4 END ASC, id DESC', {
...
RegisterServerEvent("sadiqBilling:PayInvoice")
AddEventHandler("sadiqBilling:PayInvoice", function(invoice_id)
	local xPlayer = QBCore.Functions.GetPlayer(source)

	MySQL.Async.fetchAll('SELECT * FROM sadiqBilling WHERE id = @id', {
		['@id'] = invoice_id
	}, function(result)
		...
		--[[if playerMoney < invoices.invoice_value then
			...
			if playerMoney < invoice.inoice_value then 
				...
				TriggerEvent("sadiqBilling:server:addAccountMoney", invoices.society, function(account)
				if account ~= nil then
					account.Functions.AddMoney(invoices.invoice_value)
				end
			end)

			MySQL.Async.execute('UPDATE sadiqBilling SET status = @status, paid_date = CURRENT_TIMESTAMP WHERE id = @id', {
				['@status'] = 'paid',
				['@id'] = invoice_id
			})
...
RegisterServerEvent("sadiqBilling:CancelInvoice")
AddEventHandler("sadiqBilling:CancelInvoice", function(invoice_id)
	local xPlayer = QBCore.Functions.GetPlayer(source)
	...
	MySQL.Async.fetchAll('SELECT * FROM sadiqBilling WHERE id = @id', {
		...
		MySQL.Async.execute('UPDATE sadiqBilling SET status = "cancelled", paid_date = CURRENT_TIMESTAMP WHERE id = @id', {
			['@id'] = invoice_id
		})
...
RegisterServerEvent("sadiqBilling:CreateInvoice")
AddEventHandler("sadiqBilling:CreateInvoice", function(data)
	local _source = QBCore.Functions.GetPlayer(source)
	local target = QBCore.Functions.GetPlayer(data.target)
	local webhookData = {}

	MySQL.Async.fetchAll('SELECT id FROM sadiqBilling WHERE id = (SELECT MAX(id) FROM sadiqBilling)', {}, function(result)
		webhookData = {
			id = result[1].id + 1,
...
QBCore.Functions.CreateCallback("sadiqBilling:GetSocietyInvoices", function(source, cb, society)
	local xPlayer = QBCore.Functions.GetPlayer(source)

	MySQL.Async.fetchAll('SELECT * FROM sadiqBilling WHERE society_name = @society ORDER BY id DESC', {
		['@society'] = society
	}, function(result)
		local invoices = {}
...
function checkTimeLeft()
	MySQL.Async.fetchAll('SELECT *, TIMESTAMPDIFF(HOUR, limit_pay_date, CURRENT_TIMESTAMP()) AS "timeLeft" FROM sadiqBilling WHERE status = @status', {
		['@status'] = 'unpaid'
	}, function(result)
		for k, v in ipairs(result) do
				...
				for k, vl in pairs(whenToAddFees) do
					if v.fees_amount == k - 1 then
						if v.timeLeft >= vl*(-1) then
							MySQL.Async.execute('UPDATE sadiqBilling SET fees_amount = @fees_amount, invoice_value = @invoice_value WHERE id = @id', {
								['@fees_amount'] = k,
			...
			elseif v.timeLeft >= 0 and Config.PayAutomaticallyAfterLimit then
				local xPlayer = QBCore.Functions.GetPlayerentifier(v.receiver_identifier)
				local webhookData = {
					id = v.id,
						...
						MySQL.Async.execute('UPDATE users SET accounts = @playerAccount WHERE identifier = @target', {
							...
							TriggerEvent("esx_addonaccount:getSharedAccount", v.society, function(account2)
								if account2 ~= nil then
									...
									MySQL.Async.execute('UPDATE sadiqBilling SET status = @paid, paid_date = CURRENT_TIMESTAMP() WHERE id = @id', {
										['@paid'] = 'autopaid',
										['@id'] = v.id
									})
								end
